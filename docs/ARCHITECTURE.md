# NzRouter Architecture

_Last updated: 2026-02-06_

## Executive Summary

NzRouter is a local AI routing gateway and dashboard built on Next.js.
...
flowchart LR
    ...
    subgraph Router[NzRouter Local Process]
...
- `src/lib/localDb.js`
- file: `${DATA_DIR}/db.json` (or `~/.NzRouter/db.json` when `DATA_DIR` is unset)
- entities: providerConnections, providerNodes, modelAliases, combos, apiKeys, settings, pricing

Usage DB:

- `src/lib/usageDb.js`
- files: `~/.NzRouter/usage.json`, `~/.NzRouter/log.txt`
...
- main state: `${DATA_DIR}/db.json` (or `~/.NzRouter/db.json`)
- usage stats: `~/.NzRouter/usage.json`
- request log lines: `~/.NzRouter/log.txt`
...
flowchart LR
    ...
    subgraph ContainerOrProcess[NzRouter Runtime]
        Next[Next.js Server\nPORT=20514]
...
1. `usageDb` currently stores under `~/.NzRouter` and does not follow `DATA_DIR`.
...
- Build from source: `cd /root/dev/NzRouter && npm run build`
- Build Docker image: `cd /root/dev/NzRouter && docker build -t NzRouter .`
- Start service and verify:
- `GET /api/settings`
- `GET /api/v1/models`
- CLI target base URL should be `http://<host>:20514/v1` when `PORT=20514`
