---
name: nzrouter-web-fetch
description: Fetch URL → markdown / text / HTML via NzRouter /v1/web/fetch using Firecrawl / Jina Reader / Tavily Extract / Exa Contents. Use when the user wants to scrape a webpage, extract URL content, read article, or convert a URL to markdown.
---
...
```bash
curl $NZROUTER_URL/v1/models/web | jq '.data[] | select(.kind=="webFetch") | .id'
# Per-provider params
curl "$NZROUTER_URL/v1/models/info?id=firecrawl/fetch"
```
...
`POST $NZROUTER_URL/v1/web/fetch`
...
### Jina Reader
```bash
curl -X POST $NZROUTER_URL/v1/web/fetch \
  -H "Authorization: Bearer $NZROUTER_KEY" \
  -H "Content-Type: application/json" \
  -d '{"model":"jina-reader","url":"https://nzrouter.com","format":"markdown"}'
```
...
```js
const r = await fetch(`${process.env.NZROUTER_URL}/v1/web/fetch`, {
  method: "POST",
