...
const previousDefaultRegExp = /^# nzrouter-prev-default = "([^"]*)"[ \t]*\r?\n?/m;
const previousSubagentRegExp = (type) =>
  new RegExp(
    `^# nzrouter-prev-subagent-${escapeRegExp(type)} = "([^"]*)"[ \\t]*\\r?\\n?`,
    "m",
  );
...
function buildModelSection({ slot, model, baseUrl, apiKey, contextWindow, name }) {
  const lines = [
    ...
    `description = ${tomlString("Routed via NzRouter gateway")}`,
...
function rememberPreviousDefault(toml) {
  ...
  return insertMarker(toml, `# nzrouter-prev-default = ${tomlString(current)}\n`);
}
...
function rememberPreviousSubagent(toml, type) {
  ...
  return insertMarker(
    toml,
    `# nzrouter-prev-subagent-${type} = ${tomlString(previous)}\n`,
  );
}
