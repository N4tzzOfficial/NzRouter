...
// Collect PIDs of all nzrouter-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from nzrouter install dir or wrapper cli.js
        const isAppProcess = lower.includes("nzrouter") ||
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("nzrouter") ||
...
function getDataDir() {
  ...
  if (process.platform === "win32") {
    return path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "nzrouter");
  }
  return path.join(os.homedir(), ".nzrouter");
}
...
// Resolve npx/nzrouter binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx nzrouter` — works regardless of global bin path changes after npm i -g
