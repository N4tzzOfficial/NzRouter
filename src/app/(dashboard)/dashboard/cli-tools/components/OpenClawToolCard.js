...
export default function OpenClawToolCard({
  ...
  const currentBaseUrl = openclawStatus?.settings?.models?.providers?.["nzrouter"]?.baseUrl || "";

  const getConfigStatus = () => {
    if (!openclawStatus?.installed) return null;
    const currentProvider = openclawStatus.settings?.models?.providers?.["nzrouter"];
  ...
  useEffect(() => {
    if (initialStatusRef.current) {
      setOpenclawStatus(initialStatusRef.current);
    ...
    const load = async () => {
      if (!openclawStatus) {
        try {
          ...
          if (!cancelled) setOpenclawStatus({ installed: false });
        }
      }
  ...
  }, [isExpanded, openclawStatus, fetchModelAliases]);

  // Initialize model/agent data from status - only on first load after status is installed
  useEffect(() => {
    if (openclawStatus?.installed && !hasInitializedModel.current) {
      hasInitializedModel.current = true;
      const provider = openclawStatus.settings?.models?.providers?.["nzrouter"];
      if (provider) {
        const primaryModel = openclawStatus.settings?.agents?.defaults?.model?.primary;
        if (primaryModel) setTimeout(() => setSelectedModel(primaryModel.replace("nzrouter/", "")), 0);
        if (provider.apiKey && apiKeys?.some(k => k.key === provider.apiKey)) {
          setTimeout(() => setSelectedApiKey(provider.apiKey), 0);
        }
      }
      ...
      setTimeout(() => setAgentModels(initAgentModels), 0);
    }
  ...
  const getManualConfigs = () => {
    const keyToUse = (selectedApiKey && selectedApiKey.trim())
      ? selectedApiKey
      : (!cloudEnabled ? "sk_nzrouter" : "<API_KEY_FROM_DASHBOARD>");

    const settingsContent = {
      agents: {
        defaults: {
          model: {
            primary: `nzrouter/${selectedModel || "provider/model-id"}`,
          },
        },
      },
      models: {
              ...
              <div className="flex flex-col gap-3 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-text-muted">Manual configuration is still available if NzRouter is deployed on a remote server.</p>
                  </div>
                </div>
          ...
          {!checkingOpenclaw && openclawStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {openclawStatus?.settings?.models?.providers?.["nzrouter"]?.baseUrl && (
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleResetSettings} disabled={!openclawStatus?.hasNzRouter} loading={restoring}>
