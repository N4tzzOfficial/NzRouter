...
async function getInternalHeaders() {
  ...
  headers["x-nzr-cli-token"] = await getConsistentMachineId(CLI_TOKEN_SALT);
  // Legacy header for rolling deploys still running old dashboardGuard
  headers["x-9r-cli-token"] = headers["x-nzr-cli-token"];
  return headers;
}
