...
const getProviderEnvPath = () => {
  ...
  return path.join(configDir, "jcode", "provider-nzrouter.env");
};
...
const hasNzRouterConfig = (config) => {
  ...
  if (providers["nzrouter"]) return true;

  for (const [name, provider] of Object.entries(providers)) {
    if (provider.base_url && provider.base_url.includes("localhost:20514")) {
      return true;
    }
  }
...
export async function GET() {
  ...
  const hasNzRouter = hasNzRouterConfig(config);
...
export async function POST(request) {
  try {
    ...
    env.JCODE_NZROUTER_API_KEY = apiKey;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "jcode configured successfully. Use: jcode --provider-profile nzrouter",
      configPath: getConfigPath(),
    });
  } catch (error) {
...
export async function DELETE() {
  try {
    ...
    delete config.providers["nzrouter"];

    await writeConfig(config);

    const env = await readProviderEnv();
    delete env.JCODE_NZROUTER_API_KEY;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "NzRouter configuration removed from jcode",
    });
  } catch (error) {
