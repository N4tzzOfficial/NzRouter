...
// Check if settings has NzRouter customModels
const hasNzRouterConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id?.startsWith("custom:NzRouter"));
};
...
// POST - Update NzRouter customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing NzRouter configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:NzRouter"));
...
// DELETE - Remove NzRouter customModels only (keep other settings)
