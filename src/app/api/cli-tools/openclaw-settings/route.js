...
// Check if settings has NzRouter config
const hasNzRouterConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["nzrouter"];
};

// Read per-agent models.json and return current model id (without "nzrouter/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["nzrouter"]?.models;
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasNzRouter: hasNzRouterConfig(settings),
...
// POST - Update NzRouter settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `nzrouter/${model}`;

    // Remove all old nzrouter/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("nzrouter/"))
    ...
    // Add fresh NzRouter models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`nzrouter/${m}`] = {};
    });

    // Remove old nzrouter model from each agent in agents.list. The
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (resolveAgentModel(agent.model).startsWith("nzrouter/")) {
    ...
    // Update models.providers.nzrouter with all models
    settings.models.providers["nzrouter"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `nzrouter/${agentModel}` };
        return agent;
      });
...
// DELETE - Remove NzRouter settings only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove NzRouter from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["nzrouter"];
    ...
    // Remove nzrouter models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("nzrouter/"));
    ...
    // Reset agents.defaults.model.primary if it uses nzrouter
    if (settings.agents?.defaults?.model?.primary?.startsWith("nzrouter/")) {
    ...
    return NextResponse.json({
      success: true,
      message: "NzRouter settings removed successfully",
    });
  } catch (error) {
