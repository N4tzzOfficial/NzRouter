...
const hasNzRouterConfig = (config) => {
  if (!config?.provider) return false;
  return !!config.provider["nzrouter"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["nzrouter"];
    const modelMap = providerConfig?.models || {};

    return NextResponse.json({
      installed: true,
      config,
      hasNzRouter: hasNzRouterConfig(config),
      configPath: getConfigPath(),
        opencode: {
          models: Object.keys(modelMap),
          activeModel: config?.model?.startsWith("nzrouter/") ? config.model.replace(/^nzrouter\//, "") : null,
...
// POST - Apply NzRouter as openai-compatible provider (multi-model support)
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_nzrouter";
    const effectiveSubagentModel = subagentModel || modelsArray[0];

    // Ensure provider object
    if (!config.provider) config.provider = {};

    // Preserve any existing nzrouter provider entry and its models
    const existingProvider = config.provider["nzrouter"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["nzrouter"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `nzrouter/${finalActive}`;
      }
    }
    ...
    config.agent.explorer = {
      ...
      mode: "subagent",
      model: `nzrouter/${effectiveSubagentModel}`,
    };
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("nzrouter/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove NzRouter provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["nzrouter"]?.models) {
      delete config.provider["nzrouter"].models[modelToRemove];
      
      // If no models left, remove the provider
      if (Object.keys(config.provider["nzrouter"].models).length === 0) {
        delete config.provider["nzrouter"];
        if (config.model?.startsWith("nzrouter/")) delete config.model;
      } else if (config.model === `nzrouter/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["nzrouter"].models);
        config.model = `nzrouter/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire nzrouter provider
      if (config.provider) delete config.provider["nzrouter"];
      if (config.model?.startsWith("nzrouter/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("nzrouter/")) {
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "NzRouter settings removed from OpenCode",
    });
  } catch (error) {
