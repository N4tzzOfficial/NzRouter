...
const hasNzRouterConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "NzRouter");
};

const getNzRouterEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "NzRouter") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getNzRouterEntry(config);
...
// POST - Apply NzRouter config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing NzRouter entry or append
    const idx = config.findIndex((e) => e.name === "NzRouter");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove NzRouter entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "NzRouter");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "NzRouter removed from Copilot config",
    });
  } catch (error) {
