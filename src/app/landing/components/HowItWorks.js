"use client";

export default function HowItWorks() {
  return (
    <section className="py-24 border-y border-border bg-surface/30" id="how-it-works">
      <div className="max-w-7xl mx-auto px-6">
        <div className="mb-16">
          <h2 className="text-3xl md:text-4xl font-black mb-4">How NzRouter Works</h2>
          <p className="text-text-muted max-w-xl text-lg">
            Data flows seamlessly from your application through our intelligent routing layer to the best provider for the job.
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative">
          {/* Connection line */}
          <div className="hidden md:block absolute top-12 left-[16%] right-[16%] h-[2px] bg-gradient-to-r from-transparent via-primary to-transparent -z-10"></div>

          {/* Step 1: CLI & SDKs */}
          <div className="flex flex-col gap-6 relative group">
            <div className="w-24 h-24 rounded-2xl bg-bg border border-border flex items-center justify-center shadow-xl group-hover:border-primary/50 transition-colors z-10 mx-auto md:mx-0 card-glass">
              <span className="material-symbols-outlined text-4xl text-text-muted">terminal</span>
            </div>
            <div>
              <h3 className="text-xl font-bold mb-2">1. CLI & SDKs</h3>
              <p className="text-sm text-text-muted">
                Your requests start from your favorite tools or our unified SDK. Just change the base URL.
              </p>
            </div>
          </div>

          {/* Step 2: NzRouter Hub */}
          <div className="flex flex-col gap-6 relative group md:items-center md:text-center">
            <div className="w-24 h-24 rounded-2xl bg-bg border-2 border-primary flex items-center justify-center shadow-[0_0_30px_rgba(0,180,255,0.2)] z-10 mx-auto card-glass">
              <span className="material-symbols-outlined text-4xl text-primary animate-pulse">hub</span>
            </div>
            <div>
              <h3 className="text-xl font-bold mb-2 text-primary">2. NzRouter Hub</h3>
              <p className="text-sm text-text-muted">
                Our engine analyzes the prompt, checks provider health, and routes for lowest latency or cost.
              </p>
            </div>
          </div>

          {/* Step 3: AI Providers */}
          <div className="flex flex-col gap-6 relative group md:items-end md:text-right">
            <div className="w-24 h-24 rounded-2xl bg-bg border border-border flex items-center justify-center shadow-xl group-hover:border-primary/50 transition-colors z-10 mx-auto md:mx-0 card-glass">
              <div className="grid grid-cols-2 gap-2">
                <div className="w-6 h-6 rounded bg-primary/30"></div>
                <div className="w-6 h-6 rounded bg-purple-500/30"></div>
                <div className="w-6 h-6 rounded bg-green-500/30"></div>
                <div className="w-6 h-6 rounded bg-amber-500/30"></div>
              </div>
            </div>
            <div>
              <h3 className="text-xl font-bold mb-2">3. AI Providers</h3>
              <p className="text-sm text-text-muted">
                The request is fulfilled by OpenAI, Anthropic, Gemini, or others instantly.
              </p>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}