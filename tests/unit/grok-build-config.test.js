...
describe("grokBuildConfig", () => {
  it("creates independent main and per-type subagent model slots", () => {
    ...
    expect(parsed.default).toBe("NzRouter");
    expect(parsed.model).toMatchObject({
      model: "cx/gpt-5.6-sol",
      base_url: "http://127.0.0.1:20514/v1",
      context_window: 400000,
    });
    expect(parsed.subagentMappings).toMatchObject({
      "general-purpose": "NzRouter-general-purpose",
      explore: "NzRouter-explore",
      plan: "grok-4.5",
    });
  ...
  it("is idempotent and updates owned slots without duplicate sections", () => {
    ...
    expect(result.match(/^\[model\.NzRouter\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\.NzRouter-general-purpose\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\.NzRouter-explore\]$/gm)).toHaveLength(1);
    expect(result.match(/^# NzRouter-prev-subagent-explore/gm)).toHaveLength(1);
  ...
  it("blank override restores previous subagent mapping and removes owned slot", () => {
    ...
    expect(result).not.toContain("[model.NzRouter-explore]");
    expect(parsed.subagentMappings["general-purpose"]).toBe("NzRouter-general-purpose");
  });
  ...
  it("reset restores previous default and all previous subagent mappings", () => {
    ...
    expect(reset).not.toContain("[model.NzRouter-");
    expect(reset).not.toContain("NzRouter-prev-");
  ...
  it("removes mappings that were originally unset", () => {
    ...
    expect(parseGrokBuildConfig(applied).subagentMappings.plan).toBe("NzRouter-plan");
  ...
  it("legacy callers without subagentModels leave existing overrides untouched", () => {
    ...
    expect(parsed.subagentMappings.explore).toBe("NzRouter-explore");
  ...
  it("returns stable slot names only for supported subagent types", () => {
    expect(getGrokSubagentSlot("general-purpose")).toBe("NzRouter-general-purpose");
    expect(getGrokSubagentSlot("explore")).toBe("NzRouter-explore");
    expect(getGrokSubagentSlot("plan")).toBe("NzRouter-plan");
