...
async function buildCodexHeader() {
  ...
  const { installed, hasNzRouter, config } = result.data;
...
async function buildDroidHeader() {
  ...
  const { installed, hasNzRouter, settings } = result.data;
  ...
  // Extract NzRouter custom model config
  const custom = settings?.customModels?.find(m => m.id === "custom:NzRouter-0");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenClawHeader() {
  ...
  // Extract NzRouter provider config
  const provider = settings?.models?.providers?.["nzrouter"];
  const primary = settings?.agents?.defaults?.model?.primary || "";
  const model = primary.startsWith("nzrouter/") ? primary.replace("nzrouter/", "") : (provider?.models?.[0]?.id || "");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenCodeHeader() {
  ...
  const { installed, hasNzRouter, opencode } = result.data;
...
async function showCliToolsMenu(port, breadcrumb = []) {
  ...
  await showMenuWithBack({
    title: "🔧 CLI Tools",
    breadcrumb,
    headerContent: `Configure CLI tools to use NzRouter\nEndpoint: ${endpoint}`,
    items: [
      {
        label: "Claude Code",
