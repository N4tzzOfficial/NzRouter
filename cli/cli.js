...
// Subcommands (`nzrouter xai video …`) run against an already-running gateway
...
// Self-heal SQLite runtime deps (sql.js + better-sqlite3) into ~/.nzrouter/runtime
...
// Identifiers for killAllAppProcesses - only kill nzrouter specifically
const PROCESS_IDENTIFIERS = [
  'nzrouter'  // Only package name - avoid killing other apps
];
...
function getAppDataDir() {
  return process.platform === "win32"
    ? path.join(process.env.APPDATA || "", "nzrouter")
    : path.join(os.homedir(), ".nzrouter");
}
    ...
    try {
      ...
      if (platform === "win32") {
        ...
        try {
          ...
          const lines = output.split("\n").slice(1).filter(l => l.trim());
          lines.forEach(line => {
            // Whitelist: real node process running nzrouter/cli.js, or next-server.
            // Avoids killing editors/grep/strace/cursor that just have "nzrouter" in cmdline.
            const cmd = line.toLowerCase();
            const isAppProcess =
              (cmd.includes("node") && cmd.includes("nzrouter") && (cmd.includes("cli.js") || cmd.includes("\\nzrouter") || cmd.includes("/nzrouter")))
        ...
        try {
          ...
          lines.forEach(line => {
            // Whitelist: real node process running nzrouter/cli.js, or next-server.
            // Avoids killing grep/strace/editors/cursor that incidentally match "nzrouter".
            const cmd = line.toLowerCase();
            const isAppProcess =
              (cmd.includes("node") && cmd.includes("nzrouter") && (cmd.includes("cli.js") || cmd.includes("/nzrouter")))
              || cmd.includes("next-server");
            if (isAppProcess) {
              const parts = line.trim().split(/\s+/);
              const pid = parts[1];
...
function startServer(updatePromise) {
      ...
      while (true) {
        ...
        if (choice === "update") {
          ...
          if (process.platform === "darwin") {
            ...
            console.log(`🔔 NzRouter is running in tray (PID: ${process.pid})`);
            console.log(`   Server: http://${displayHost}:${port}`);
          ...
          console.log(`🔔 NzRouter is now running in background (PID: ${bgProcess.pid})`);
          console.log(`   Server: http://${displayHost}:${port}`);
          console.log(`\n💡 You can close this terminal. Right-click tray icon to quit.\n`);
  ...
  function tryRestart(code) {
    ...
    if (restartCount >= MAX_RESTARTS) {
      ...
      try {
        const dbPath = path.join(os.homedir(), process.platform === "win32" ? path.join("AppData", "Roaming", "nzrouter", "db.json") : path.join(".nzrouter", "db.json"));
