...
将 NzRouter 与 Cline VSCode 扩展集成,通过 NzRouter 的智能路由系统转发你的 AI 请求。
...
将 base URL 设为你的 NzRouter endpoint:
...
**步骤:**
1. 在 **Base URL** 字段中输入你的 NzRouter endpoint
2. 末尾必须包含 `/v1`
...
3. key 应以 `sk-NzRouter-` 开头

### 5. 选择模型

1. 在 **Model** 下拉菜单中,可以:
   - 从可用模型中选择(若 Cline 自动检测)
   - 或手动输入 NzRouter 配置中的模型名
...
2. 在聊天输入框中输入消息
3. 按 Enter 发送
4. Cline 会通过 NzRouter 处理你的请求

### 生成代码

1. 让 Cline 生成代码:"创建一个登录表单的 React 组件"
2. Cline 会通过 NzRouter 生成代码
3. 检查并接受生成的代码
...
1. 在编辑器中选中代码
2. 让 Cline:"解释这段代码"
3. 通过 NzRouter 获得 AI 驱动的解释

### 文件操作

1. 让 Cline 创建、修改或删除文件
2. Cline 会通过 NzRouter 理解上下文并进行修改
3. 在接受前检查变更
...
1. 在 Cline 设置中将 Base URL 设为:`https://NzRouter.com`
2. 确保已在 NzRouter 云端仪表盘中配置 API key
...
某个提供商失败时,NzRouter 会自动回退到仪表盘中配置的备用提供商。

### 使用跟踪

通过 NzRouter 仪表盘监控你的 Cline 使用:
- 请求总数
- Token 使用
- 每个模型的成本
- 提供商分布
